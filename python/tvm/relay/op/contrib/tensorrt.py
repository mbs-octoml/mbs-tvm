...
from tvm.relay.expr import Call, Constant, GlobalVar, Tuple, TupleGetItem
...
def is_supported_trt_type(type, op_name):  # pylint: disable=redefined-builtin
    supported_dtypes = ["float32", "float16"]
    if isinstance(type, tvm.ir.TensorType):
        if type.dtype not in supported_dtypes:
        ...
        logger.info(f"{op_name}: Type {type} is not supported.")
        return False
    return True
...
def make_trt_version_checker(version):
    """Helper for ops which require a minimum TRT version"""

    def checker(attrs, args, op_name):
        if get_tensorrt_version() < version:
            logger.info(f"{op_name}: requires TensorRT version {'.'.join(map(str, version))} or higher.")
            return False
        return True
...
def multiply_checker(attrs, args, op_name):
    shapes = [
        [
...
def reduce_checker(attrs, args, op_name):
...
def add_checker(attrs, args, op_name):  # pylint: disable=unused-variable
...
def batch_norm_checker(attrs, args, op_name):  # pylint: disable=unused-variable
...
def softmax_checker(attrs, args, op_name):  # pylint: disable=unused-variable
...
def conv1d_checker(attrs, args, op_name):  # pylint: disable=unused-variable
...
def conv2d_checker(attrs, args, op_name):  # pylint: disable=unused-variable
...
def dense_checker(attrs, args, op_name):  # pylint: disable=unused-variable
...
def batch_matmul_checker(attrs, args, op_name):  # pylint: disable=unused-variable
    """Check if dense is supported by TensorRT."""
...
def layer_norm_checker(attrs, args, op_name):
    """Check if dense is supported by TensorRT."""
    if get_tensorrt_use_implicit_batch_mode() and int(attrs.axis) == 0:
        logger.info(f"{op_name}: requires use_implict_batch=False.")
        return False
...
def bias_add_checker(attrs, args, op_name):  # pylint: disable=unused-variable
...
def max_pool_2d_checker(attrs, args, op_name):  # pylint: disable=unused-variable
...
def avg_pool_2d_checker(attrs, args, op_name):  # pylint: disable=unused-variable
...
def global_max_pool_2d_checker(attrs, args, op_name):  # pylint: disable=unused-variable
...
def global_avg_pool_2d_checker(attrs, args, op_name):  # pylint: disable=unused-variable
...
def expand_dims_checker(attrs, args, op_name):  # pylint: disable=unused-variable
...
def squeeze_checker(attrs, args, op_name):  # pylint: disable=unused-variable
...
def concatenate_checker(attrs, args, op_name):  # pylint: disable=unused-variable
...
def split_checker(attrs, args, op_name):  # pylint: disable=unused-variable
...
def conv2d_transpose_checker(attrs, args, op_name):  # pylint: disable=unused-variable
...
def transpose_checker(attrs, args, op_name):  # pylint: disable=unused-variable
...
def layout_transform_checker(attrs, args, op_name):  # pylint: disable=unused-variable
...
def reshape_checker(attrs, args, op_name):  # pylint: disable=unused-variable
...
def pad_checker(attrs, args, op_name):  # pylint: disable=unused-variable
...
def strided_slice_checker(attrs, args, op_name):  # pylint: disable=unused-variable
...
def adaptive_max_pool2d_checker(attrs, args, op_name):  # pylint: disable=unused-variable
...
def adaptive_avg_pool2d_checker(attrs, args, op_name):  # pylint: disable=unused-variable
...
def conv3d_checker(attrs, args, op_name):  # pylint: disable=unused-variable
...
def max_pool_3d_checker(attrs, args, op_name):  # pylint: disable=unused-variable
...
def avg_pool_3d_checker(attrs, args, op_name):  # pylint: disable=unused-variable
...
def conv3d_transpose_checker(attrs, args, op_name):  # pylint: disable=unused-variable
...
def proj_five_op_pattern_with_const(op):
    return is_tuple_get_item(is_op(op)(wildcard(), is_constant(), is_constant(), is_constant(), is_constant()), 0)
...
def pattern_table():
    ...
    return [
        ("tensorrt.nn.conv3d", binary_op_pattern_with_const("nn.conv3d"),
         make_predicate(make_and_checker(make_trt_version_checker((6, 0, 1)), conv3d_checker))),
        ("tensorrt.nn.conv2d", binary_op_pattern_with_const("nn.conv2d"), make_predicate(conv2d_checker)),
        ("tensorrt.nn.conv1d", binary_op_pattern_with_const("nn.conv1d"), make_predicate(conv1d_checker)),
        ("tensorrt.nn.conv2d_transpose", binary_op_pattern("nn.conv2d_transpose"),
        ...
        ("tensorrt.nn.dense", binary_op_pattern_with_const("nn.dense"), make_predicate(dense_checker)),
        ("tensorrt.bias_add", binary_op_pattern("nn.bias_add"), make_predicate(bias_add_checker)),
        ("tensorrt.nn.batch_matmul", binary_op_pattern("nn.batch_matmul"), make_predicate(batch_matmul_checker)),
        ...
        ("tensorrt.nn.leaky_relu", unary_op_pattern("nn.leaky_relu"),
        ...
        ("tensorrt.sin", unary_op_pattern("sin"), make_predicate(make_trt_version_checker((5, 1, 5)))),
        ("tensorrt.cos", unary_op_pattern("cos"), make_predicate(make_trt_version_checker((5, 1, 5)))),
        ("tensorrt.atan", unary_op_pattern("atan"), make_predicate(make_trt_version_checker((5, 1, 5)))),
        ("tensorrt.ceil", unary_op_pattern("ceil"), make_predicate(make_trt_version_checker((5, 1, 5)))),
        ("tensorrt.floor", unary_op_pattern("floor")),
        ("tensorrt.erf", unary_op_pattern("erf"), make_predicate(make_trt_version_checker((7, 0, 0)))),
        ...
        ("tensorrt.variance", unary_op_pattern("variance"), make_predicate(reduce_checker)),
        ("tensorrt.concatenate", unary_op_pattern_with_any_tuple("concatenate"), make_predicate(concatenate_checker)),
        ("tensorrt.expand_dims", unary_op_pattern("expand_dims"), make_predicate(expand_dims_checker)),
        ("tensorrt.layout_transform", unary_op_pattern("layout_transform"), make_predicate(layout_transform_checker)),
        ...
        ("tensorrt.strided_slice", unary_op_pattern("strided_slice"),
         make_predicate(make_and_checker(make_trt_version_checker((5, 1, 5)), strided_slice_checker))),
        ("tensorrt.nn.adaptive_avg_pool2d", unary_op_pattern("nn.adaptive_avg_pool2d"),
         make_predicate(adaptive_avg_pool2d_checker)),
        ("tensorrt.nn.adaptive_max_pool2d", unary_op_pattern("nn.adaptive_max_pool2d"),
         make_predicate(adaptive_max_pool2d_checker)),
        ("tensorrt.nn.max_pool3d", unary_op_pattern("nn.max_pool3d"),
         make_predicate(make_and_checker(make_trt_version_checker((6, 0, 1)), max_pool_3d_checker))),
        ("tensorrt.nn.avg_pool3d", unary_op_pattern("nn.avg_pool3d"),
         make_predicate(make_and_checker(make_trt_version_checker((6, 0, 1)), avg_pool_3d_checker))),
        ("tensorrt.nn.conv3d_transpose", unary_op_pattern("nn.conv3d_transpose"),
         make_predicate(make_and_checker(make_trt_version_checker((6, 0, 1)), conv3d_transpose_checker))),
        ("tensorrt.nn.softmax", unary_op_pattern("nn.softmax"), make_predicate(softmax_checker)),
        ("tensorrt.nn.layer_norm", unary_op_pattern("nn.layer_norm"), make_predicate(layer_norm_checker)),
        ("tensorrt.nn.max_pool2d", unary_op_pattern("nn.max_pool2d"), make_predicate(max_pool_2d_checker)),
        ("tensorrt.nn.avg_pool2d", unary_op_pattern("nn.avg_pool2d"), make_predicate(avg_pool_2d_checker)),
        ("tensorrt.nn.global_max_pool2d", unary_op_pattern("nn.global_max_pool2d"),
         make_predicate(global_max_pool_2d_checker)),
        ("tensorrt.nn.global_avg_pool2d", unary_op_pattern("nn.global_avg_pool2d"),
         make_predicate(global_avg_pool_2d_checker)),
        ("tensorrt.nn.batch_norm", proj_five_op_pattern_with_const("nn.batch_norm"),
...
class IsComputeIntensiveGraph(ExprVisitor):
    ...
    def visit_call(self, call):
        compute_intensive_ops = {"nn.conv1d", "nn.conv2d", "nn.conv2d_transpose", "nn.conv3d", "nn.conv3d_transpose",
                                 "nn.dense", "nn.batch_matmul", "sum", "prod", "max", "min", "mean"}
