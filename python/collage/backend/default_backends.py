...
def cg_TensorRT(mod, target, params, **kwargs):
    ...
    logging.info(f"After partitioning for TensorRT model is:\n{mod}")
...
def cg_op_level_backends(backend, net, target, params, **kwargs):
...
def cg_cuDNN(net, target, params, **kwargs):
    return cg_op_level_backends("cudnn", net, target, params, **kwargs)


def cg_cuBLAS(net, target, params, **kwargs):
    return cg_op_level_backends("cublas", net, target, params, **kwargs)


def cg_MKL(net, target, params, **kwargs):
    return cg_op_level_backends("mkl", net, target, params, **kwargs)


def partition_CUTLASS(mod):
    return relay.op.contrib.partition_for_cutlass(mod)


def cg_CUTLASS(mod, target, params, **kwargs):
    sm = 80   # rtx3070 is 8.6 but CUTLASS only supports 8.0
    with tvm.transform.PassContext(opt_level=3, disabled_pass=["AlterOpLayout"]):
        mod = relay.op.contrib.partition_for_cutlass(mod, params)
