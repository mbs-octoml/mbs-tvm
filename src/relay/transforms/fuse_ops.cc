...
Expr FuseOps(const Expr& expr, int fuse_opt_level, size_t max_fuse_depth, bool link_params,
  ...
  auto fused_expr =
      FuseMutator(fuse_opt_level, max_fuse_depth, link_params, is_custom_pass).Transform(expr);
...
IRModule AssignBackendFunc(IRModule module) {
  ...
  if (fn_node->GetAttr<IntImm>(attr::kCustomFusionPass).defined()) {
    ...
    LOG(INFO) << "Invoking " << custom_fusion_pass_str << " on:\n" << PrettyPrint(module);
    ...
    (*fdp_call)(expr);
    LOG(INFO) << "Done with " << custom_fusion_pass_str << " on:\n" << PrettyPrint(module);
    ...
    const auto* ex_op_call =
        tvm::runtime::Registry::Get("collage.optimizer.apply_external_compiler_op");
    ICHECK(ex_op_call);
    // Warning(@Soo): Doublecheck if module is updated.
    module = (*ex_op_call)(module);
    LOG(INFO) << "Done with collage.optimizer.apply_external_compiler_op with result:\n"
              << PrettyPrint(module);
  }
