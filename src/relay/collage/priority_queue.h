...
#ifndef SRC_RELAY_COLLAGE_PRIORITY_QUEUE_H_
#define SRC_RELAY_COLLAGE_PRIORITY_QUEUE_H_
...
class PriorityQueue {
 public:
  ...
  void Update(T* item) {
    auto itr = std::find_if(set_.begin(), set_.end(), [item](const T* that) {
      return EqTPtr()(that, item);
    });
    ICHECK(itr != set_.end());
    set_.erase(itr);
    set_.emplace(item);
...
#endif  // SRC_RELAY_COLLAGE_PRIORITY_QUEUE_H_
