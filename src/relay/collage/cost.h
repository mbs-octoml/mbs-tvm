 ...
 * \brief Represents the estimated cost of a candidate kernel or partitioning.
 */
...
/*!
 * \brief The assumed cost for a candidate kernel. Generally average execution time in seconds.
...
class Cost {
 public:
   ...
   * by the intended target, for example because the sub-graph has an unsupported operator or
   * the intermediate memory required exceeds some system limit.
   */
   ...
   * choose best the kernels. This can be used to disable tuning and fallback to fixed rules, much
   * as TVM will use an untuned kernel.
   */
 ...
 private:
   ...
   *   - +inf if candidate kernel is not feasible.
   *   - NaN if candidate kernel has an unknown cost (fusion rule priority may be used to break
   *     ties).
   */
  double value_ = 0.0;
};
