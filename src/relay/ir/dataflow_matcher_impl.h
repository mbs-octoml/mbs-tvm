...
class DFPatternMatcher : public DFPatternFunctor<bool(const DFPattern&, const Expr&)> {
 public:
  explicit DFPatternMatcher(const DataflowGraph* expr_graph) : expr_graph_(expr_graph) {}
  bool Match(const DFPattern& pattern, const Expr& expr);
  Map<DFPattern, Array<Expr>> GetMemo() { return Map<DFPattern, Array<Expr>>(memo_); }

  const DataflowGraph::Node* expr_to_node(const Expr& expr) const {
    return expr_graph_->item_to_node(expr);
  }
  const DataflowGraph::Node* index_to_node(size_t index) const {
  ...
  const DataflowGraph& dataflow_graph() const { return *expr_graph_; }
 ...
 protected:
  ...
  const DataflowGraph* expr_graph_;
...
class PatternGrouper {
 ...
 protected:
  ...
  std::unique_ptr<PatternGraph> pattern_graph_;
  int gid_ = 0;
  int graph_number_ = 0;
};
